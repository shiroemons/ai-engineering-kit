...
初期の全文検索は AND 条件の部分一致で、クエリ内のすべての語が文書に含まれる必要がある。eval の検索語は完成した文書のタイトル・タグ・本文から選び、類義語や翻訳語だけに頼らない。各クエリを `go run ./cmd/kb search "<検索語>" --json` で実行し、期待 ID が返ることを確認する。
