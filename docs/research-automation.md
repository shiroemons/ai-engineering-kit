...
`just research-loop` は現在の領域・テーマ・調査段階と、テーマ単位の推定進捗率を端末に表示する。進捗は8時間の実行時間に対する割合ではない。テーマ選定で10%、一次資料の確認後に30%、knowledge 文書の完成後に55%、検索 eval の作成後に70%、検索結果の確認後に85%、runner 検証後に95%、成果の統合後に100%となる。10〜90%は調査エージェントが完了を報告した段階、95〜100%はrunnerが確認した段階を示すため、厳密な作業量や内容の完全性ではなく目安として扱う。失敗時には止まった段階と理由を端末・`research-error.log` に記録する。
