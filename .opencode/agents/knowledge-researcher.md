...
Only in `PHASE: EVAL_AND_VALIDATION`, run `just validate` before finishing. If verification fails or sources are weak, report the reason and stop. End that phase with `TOPIC: <technology and topic>` on its own line and a short summary of source URLs, files changed, and any unverified questions.

The runner uses four bounded phases. Each phase is a separate model invocation, and its completion marker ends that phase. Execute only the active phase; do not repeat completed phases or attempt later phases in the same invocation. These phase instructions override the full-run descriptions above.
...
In `PHASE: EVAL_AND_VALIDATION`, keep the selected topic fixed. Write the assigned eval, run each new query, and run `just validate`. Emit each requested progress milestone only after it is complete. Emit final `TOPIC: <technology and topic>` exactly once, as the final line, only after validation passes. If a required step fails, report the concrete reason and do not emit `TOPIC`.
